...
interface AuthenticatedUser { organizationId: string }
...
export class EventGroupController {
  ...
  @Roles('OWNER')
  @Post()
  create(@CurrentUser() user: AuthenticatedUser, @Body() data: CreateEventGroupDto) {
    return this.eventGroupService.create(user.organizationId, data);
  }

  @Roles('OWNER')
  @Patch(':id')
  update(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser, @Body() data: UpdateEventGroupDto) {
    return this.eventGroupService.update(user.organizationId, id, data);
  }

  @Roles('OWNER')
  @Put(':id/events')
  replaceEvents(@Param('id') id: string, @CurrentUser() user: AuthenticatedUser, @Body() data: UpdateEventGroupEventsDto) {
