...
describe('PublicBookingService public rule evaluation', () => {
  ...
  it('should require one Kanga for an age-4 participant', async () => {
      ...
      requiredProducts: [
        {
          ...
          quantity: 1,
          ruleId: 'rule-kanga-age-3-5',
          ruleName: 'Age 3-5 Requires Kanga',
          message: 'Children aged 3 to 5 require a Kanga Skating Aid.',
        },
      ],
      errors: [],
      warnings: [],
    ...
    const result = await service.evaluateRules('event-1', {
      sessionId: 'session-1',
    ...
    expect(ruleEvaluationService.evaluate).toHaveBeenCalledWith('event-1', {
      customerAge: 4,
      participantAge: 4,
      participantFirstName: 'Young',
      participantLastName: 'Skater',
      ticketTypeId: 'ticket-child',
      sessionId: 'session-1',
      eventId: 'event-1',
      flexibleBooking: false,
      participantCount: 1,
      bookingTicketTypeIds: ['ticket-child'],
    });
  ...
  it('should reject rule preview for an inactive or missing public event', async () => {
    ...
    expect(ruleEvaluationService.evaluate).not.toHaveBeenCalled();
  });
