...
describe('BookingValidationService', () => {
  ...
  it('should throw when event does not exist', async () => {
    ...
    await expect((service as any).validateEvent('event-id')).rejects.toThrow(
      NotFoundException,
    );
  });
  ...
  it('should return active event', async () => {
    ...
    await expect((service as any).validateEvent('event-id')).resolves.toEqual(
      event,
    );
  });
  ...
  it('should throw when product is not assigned to session', async () => {
    ...
    await expect(
      (service as any).validateSessionProduct('session-id', 'product-id'),
    ).rejects.toThrow(BadRequestException);
  });
  ...
  it('should return active session product', async () => {
    ...
    prismaMock.sessionProduct.findUnique.mockResolvedValue(sessionProduct);
  ...
  it('should throw when product is unavailable online', () => {
    expect(() =>
      (service as any).validateChannelAvailability(
        {
  ...
  it('uses POS availability for walk-up bookings', () => {
    expect(() =>
      (service as any).validateChannelAvailability(
        {
          availableOnline: false,
