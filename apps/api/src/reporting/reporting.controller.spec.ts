...
describe('ReportingController', () => {
  ...
  it('uses trusted Organisation context for detailed sales reports', async () => {
    ...
    expect(service.getSessionSales).toHaveBeenCalledWith(
      'org-1',
      'event-1',
      query,
    );
    expect(service.getProductSales).toHaveBeenCalledWith(
      'org-1',
      'event-1',
      query,
    );
    expect(service.getDateSales).toHaveBeenCalledWith(
      'org-1',
      'event-1',
      query,
    );
    expect(service.getSalesPace).toHaveBeenCalledWith(
      'org-1',
      'event-1',
      query,
    );
  ...
  it('sets safe download headers for Event CSV exports', async () => {
    ...
    service.getEventCsv.mockResolvedValue({
      filename: 'event-sessions-2027-09-01.csv',
    ...
    expect(service.getEventCsv).toHaveBeenCalledWith(
      'org-1',
      'event-1',
      'sessions',
      { date: '2027-09-01' },
