      ...
      <div className={embedded ? "space-y-6" : "space-y-8"}>
        <div>
          <p className="text-sm font-medium text-muted-foreground">
            {fixedEventId ? "Event operations" : "Operations"}
          </p>
          ...
          <p className="mt-2 text-muted-foreground">
            {fixedEventId
              ? `Find bookings, payment state and ticket issuance for ${fixedEventName ?? "this Event"}.`
              : "Find customer bookings, payment state and ticket issuance across your authorised Events."}
          </p>
        </div>
                ...
                <tbody className="divide-y">
                  {bookings.map((booking) => (
                    <tr key={booking.id} className="hover:bg-muted/30">
                      ...
                      {!fixedEventId ? <td className="px-5 py-4">{booking.event.name}</td> : null}
