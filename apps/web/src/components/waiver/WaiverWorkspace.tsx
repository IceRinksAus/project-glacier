...
export function WaiverWorkspace({
  ...
  return (
    <>
          ...
          {publishedVersion && preparation?.event.status !== "ACTIVE" ? (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
              ...
              <p className="mt-1 leading-6 text-amber-900">
                This Event is still a draft. Activate the Event from its
                Overview readiness panel before sharing the public Waiver link
                or QR code.
              </p>
            </div>
          ) : null}
          ...
          <section className="rounded-2xl border bg-card p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Accepted {formatDateTime(selectedSubmission.acceptedAt)}{" "}
                  against version {selectedSubmission.waiverVersion.version}
                </p>
              </div>
              <button
                type="button"
                  ...
                  <ul className="mt-3 space-y-2 text-sm">
                    {selectedSubmission.minors.map((minor) => (
                      <li key={minor.id}>
                        ...
                        {" — "}
                        {new Date(minor.dateOfBirth).toLocaleDateString(
                          "en-AU",
                        )}
                      </li>
                    ))}
      ...
      {selectedSubmission ? (
        <SubmissionEvidencePrint
          eventName={preparation?.event.name ?? "Event"}
...
function SubmissionEvidencePrint({
  eventName,
  submission,
}: {
  eventName: string;
  submission: WaiverSubmissionDetail;
}) {
  return (
    <article className="hidden bg-white p-8 text-black print:block">
      <header className="border-b-2 border-slate-900 pb-5">
        <p className="text-xs font-bold uppercase tracking-[0.2em]">
          Glacier Waiver evidence
        </p>
        <h1 className="mt-2 text-3xl font-bold">{eventName}</h1>
        <p className="mt-2 text-sm">
          {submission.waiverVersion.title} · Version{" "}
          {submission.waiverVersion.version}
        </p>
      </header>

      <section className="mt-6 grid grid-cols-2 gap-4 text-sm">
        <EvidenceValue
          label="Adult signatory"
          value={submission.signatoryFullName}
        />
        <EvidenceValue
          label="Accepted"
          value={formatDateTime(submission.acceptedAt)}
        />
        <EvidenceValue
          label="Booking"
          value={submission.booking?.bookingNumber ?? "Not linked"}
        />
        <EvidenceValue
      ...
      <section className="mt-8 break-inside-avoid">
        <h2 className="text-lg font-bold">Accepted Waiver wording</h2>
        <div className="mt-3 whitespace-pre-wrap border p-5 text-sm leading-6">
          {submission.waiverVersion.content}
        </div>
        <div className="mt-3 border p-4 text-sm">
          <span className="font-bold">Acceptance statement:</span>{" "}
          {submission.waiverVersion.acceptanceStatement}
        </div>
      </section>

      <section className="mt-8 break-inside-avoid">
        <h2 className="text-lg font-bold">Covered people</h2>
        <p className="mt-3 text-sm">
          <span className="font-bold">Signatory:</span>{" "}
          {submission.signatoryFullName}
        </p>
        {submission.minors.length ? (
          ...
          <p className="mt-2 text-sm">No dependants recorded.</p>
        )}
      </section>

      <section className="mt-8 break-inside-avoid">
        <h2 className="text-lg font-bold">Electronic signature</h2>
        {/* The signature is private authenticated evidence and appears only in this operator export. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={submission.signatureData}
          alt={`Signature supplied by ${submission.signatoryFullName}`}
          className="mt-3 h-36 max-w-xl border object-contain"
        />
      </section>

      <section className="mt-8 break-inside-avoid border-t pt-5 text-[10px] leading-5 text-slate-600">
        <p>
          Waiver content SHA-256:{" "}
          <span className="font-mono">{submission.waiverContentHash}</span>
        </p>
        <p>
          Acceptance statement SHA-256:{" "}
          <span className="font-mono">
            {submission.acceptanceStatementHash}
          </span>
        </p>
        <p className="mt-2">
          Generated from Glacier&apos;s authenticated evidence record on{" "}
...
function EvidenceValue({
  ...
  return (
    <div>
      ...
      <p className={`mt-1 ${mono ? "font-mono text-xs" : "font-semibold"}`}>
        {value}
      </p>
    </div>
  );
