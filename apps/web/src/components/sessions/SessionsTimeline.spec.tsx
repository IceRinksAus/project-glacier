...
const { refresh, getEventReport, updateSession } = vi.hoisted(() => ({
  refresh: vi.fn(),
...
const sessions = [
  {
    ...
    endDate: "2027-06-25T02:00:00.000Z",
    capacity: 100,
    status: "ACTIVE",
    salesStart: null,
    salesEnd: null,
  ...
  },
  {
    ...
    startDate: "2027-06-25T04:00:00.000Z",
    endDate: "2027-06-25T06:00:00.000Z",
    capacity: 100,
    status: "ACTIVE",
    salesStart: null,
    salesEnd: null,
  ...
  },
  {
    ...
    startDate: "2027-06-26T00:00:00.000Z",
    endDate: "2027-06-26T02:00:00.000Z",
    capacity: 100,
    status: "DRAFT",
    salesStart: null,
    salesEnd: null,
...
vi.mock("@/hooks/useSessions", () => ({
  useSessions: () => ({ sessions, isLoading: false, error: "", refresh }),
}));
vi.mock("@/services/reporting.service", () => ({
  reportingService: { getEventReport },
}));
...
describe("SessionsTimeline", () => {
    ...
    getEventReport.mockResolvedValue({
      sessions: [
        ...
        },
        {
          id: "afternoon",
          name: "Afternoon Session",
          startDate: sessions[1].startDate,
          endDate: sessions[1].endDate,
          status: "ACTIVE",
          capacity: 100,
          reservedAttendance: 90,
        ...
        },
        {
          id: "tomorrow",
          name: "Tomorrow Session",
          startDate: sessions[2].startDate,
          endDate: sessions[2].endDate,
          status: "DRAFT",
          capacity: 100,
          reservedAttendance: 100,
          confirmedAttendance: 100,
          remainingCapacity: 0,
  ...
  it("shows a selected-day agenda with authoritative availability", async () => {
    render(
      <SessionsTimeline
        eventId="event-1"
        eventStartDate="2027-06-20T00:00:00.000Z"
        eventEndDate="2027-06-30T23:59:59.000Z"
    ...
    expect(
      screen.getByRole("progressbar", {
        name: "Morning Session reserved capacity",
      }),
  ...
  it("changes date and exposes sold-out status without relying on colour", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /26 June 2027, 1 Sessions/ }),
    );
  ...
  it("activates every draft Session in one action", async () => {
    render(
      <SessionsTimeline
        eventId="event-1"
        eventStartDate="2027-06-20T00:00:00.000Z"
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Activate all 1 draft Session" }),
    );

    await waitFor(() =>
      expect(updateSession).toHaveBeenCalledWith("tomorrow", {
        status: "ACTIVE",
      }),
    );
