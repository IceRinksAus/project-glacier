...
export function SessionsTimeline({
    ...
    try {
      ...
      setCapacityError(
        caught instanceof Error
          ? caught.message
          : "Unable to load Session availability.",
      );
    }
  }, [eventId]);
  ...
  useEffect(() => {
    if (selectedDate)
      setDisplayMonth((current) => current || selectedDate.slice(0, 7));
  }, [selectedDate]);
  ...
  const selectedSessions = useMemo(
    () =>
      [...(sessionsByDate[selectedDate] ?? [])].sort((left, right) =>
  ...
  const capacityById = useMemo(
    () => new Map(capacityRows.map((row) => [row.id, row])),
    [capacityRows],
  );
    ...
    return (
      <StateCard>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Create a schedule to populate this Event calendar.
        </p>
      </StateCard>
    );
  }

  const activeCount = selectedSessions.filter(
    (session) => session.status === "ACTIVE",
  ).length;
  const draftCount = selectedSessions.filter(
    (session) => session.status === "DRAFT",
  ).length;
  const cancelledCount = selectedSessions.filter(
    (session) => session.status === "CANCELLED",
  ).length;
  const exceptionCount = selectedSessions.filter(
    (session) =>
      session.scheduleExceptionType && session.scheduleExceptionType !== "NONE",
  ).length;
  ...
  async function activateAllDraftSessions() {
    try {
      setIsActivatingAll(true);
      setActivationError("");
      await Promise.all(
        draftSessions.map((session) =>
          sessionService.updateSession(session.id, { status: "ACTIVE" }),
        ),
      );
      await refreshAll();
    } catch (caught) {
      setActivationError(
        caught instanceof Error
          ? caught.message
          : "Unable to activate all draft Sessions.",
      );
    } finally {
        ...
        <section className="glacier-panel h-fit p-5">
          ...
          <div className="mt-5 grid grid-cols-7 gap-1 text-center">
            {weekDays.map((day) => (
              <span
                key={day}
                className="pb-2 text-[11px] font-semibold uppercase text-muted-foreground"
              >
                {day}
              </span>
            ))}
              ...
              return (
                ...
                >
                  ...
                  {count ? (
                    <span
                      aria-hidden
                      className={`absolute bottom-1 left-1/2 size-1 -translate-x-1/2 rounded-full ${selected ? "bg-primary-foreground" : "bg-primary"}`}
                    />
                  ) : null}
                </button>
              );
          ...
          <p className="mt-4 text-xs text-muted-foreground">
            Dates with a blue marker contain Sessions. All dates use {timeZone}.
          </p>
        </section>
        ...
        <div className="space-y-4">
          <section className="glacier-panel p-5">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div>
                <p className="glacier-kicker">Selected date</p>
              ...
              <div className="flex flex-wrap gap-2">
                {draftSessions.length ? (
                  <Button
                    onClick={() => void activateAllDraftSessions()}
                    disabled={isActivatingAll}
                  >
                    {isActivatingAll
                      ? "Activating..."
                      : `Activate all ${draftSessions.length} draft Session${draftSessions.length === 1 ? "" : "s"}`}
                  </Button>
                ) : null}
                <Button
                  variant="outline"
          ...
          {capacityError ? (
            ...
            >
              Session times are available, but live capacity could not be
              refreshed: {capacityError}
            </p>
          ) : null}
          {selectedSessions.length ? (
            ...
            <StateCard>
              <h3 className="font-semibold">No Sessions on this date</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                Choose a marked date or create a schedule.
              </p>
            </StateCard>
          )}
        </div>
...
function SessionRow({
  ...
  capacity?: EventReport["sessions"][number];
  timeZone: string;
  onOpen: () => void;
}) {
  ...
  const badges = {
    ...
    selling: "bg-amber-50 text-amber-900",
    limited: "bg-orange-50 text-orange-900",
    "sold-out": "bg-red-50 text-red-800",
  };
  return (
    <button
      type="button"
      onClick={onOpen}
      className="glacier-panel w-full p-5 text-left transition hover:border-primary/40 hover:shadow-md focus-visible:ring-2 focus-visible:ring-ring"
    >
      <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          ...
          <h3 className="mt-1 truncate font-semibold">{session.name}</h3>
          <div className="mt-2 flex flex-wrap gap-2 text-xs">
            ...
            session.scheduleExceptionType !== "NONE" ? (
              <span className="rounded-full bg-amber-50 px-2.5 py-1 text-amber-900">
                {session.scheduleExceptionType} schedule
              </span>
            ) : null}
          </div>
        </div>
          ...
          {availability && capacity ? (
            <>
              <div className="flex items-center justify-between gap-3 text-sm">
                <span
                  className={`rounded-full px-2.5 py-1 font-semibold ${badges[availability.tone]}`}
                >
                  {availability.label}
                </span>
                <span className="font-medium">
                  {capacity.reservedAttendance} of {capacity.capacity} reserved
                </span>
              </div>
              <div
                className="mt-3 h-2 overflow-hidden rounded-full bg-muted"
                role="progressbar"
                aria-label={`${session.name} reserved capacity`}
                aria-valuemin={0}
                aria-valuemax={capacity.capacity}
                aria-valuenow={capacity.reservedAttendance}
              >
                <div
                  className={`h-full rounded-full ${tones[availability.tone]}`}
            ...
            <p className="text-sm text-muted-foreground">
              Capacity {session.capacity} · availability loading
            </p>
          )}
        </div>
      </div>
...
function DayMetric({
  ...
  return (
    <div
      className={`rounded-lg border p-3 ${attention ? "border-amber-300 bg-amber-50" : "bg-muted/25"}`}
    >
...
function StateCard({
  ...
  return (
    <div
      className={`glacier-panel p-6 ${error ? "border-destructive/30 text-destructive" : ""}`}
    >
      {children}
    </div>
  );
