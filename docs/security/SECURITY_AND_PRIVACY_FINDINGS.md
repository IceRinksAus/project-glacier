...
| SEC-31-002 | Dependency vulnerabilities     |     High | Sprint 34 rechecked both lockfiles, applied exact patched transitive overrides for `deepmerge-ts`, `fast-uri`, `mysql2` and `qs`, and retained stable Prisma 7.9.1. No forced downgrade or prerelease framework change was used. | Development owner | Every lockfile change and before internet exposure | **Closed locally 8 September 2026**; both npm audits report zero known vulnerabilities and Prisma generation, 48 migrations, 644 API tests, 90 web tests and both builds pass. Independent supply-chain review remains separate. |
...
| SEC-31-004 | Privileged MFA and recovery    |     High | Sprint 33 implements membership-scoped TOTP enrolment, pre-session challenge, single-use recovery codes, factor rotation, bounded OWNER-to-MANAGER reset and privileged session enforcement locally. Email password recovery, production key custody/alerts and OWNER lost-factor dual-control recovery remain open. | Product and security owner | Before live operator access | Locally resolved for privileged MFA on 4 September 2026; remaining recovery/deployed evidence still blocks production activation |
