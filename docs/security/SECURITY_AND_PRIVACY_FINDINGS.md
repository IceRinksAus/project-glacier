...
| SEC-31-009 | Ticket credential at rest      |   Medium | Booking and Waiver possession credentials are hashed, while the current Ticket `secureToken` is stored as a raw unique value because Ticket presentation uses the same credential. Database compromise would therefore expose usable Ticket credentials until invalidated.                          | Development/security owner                                        | Design before internet exposure; implement before live Ticket use | Open; strict database access is required and a rotation/hash-compatible presentation design must be reviewed |
