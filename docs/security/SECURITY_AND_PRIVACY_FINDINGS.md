...
| SEC-31-002 | Dependency vulnerabilities     |  Pending | Sprint 18 evidence is stale. Fresh API and web `npm audit` checks require sending dependency metadata from each lockfile to the npm registry. This must not occur without specific organiser approval. | Development owner           | Sprint 31 close                  | Awaiting explicit approval; no current severity claim             |
...
No unresolved Critical finding is currently known from the completed local evidence. That statement is deliberately bounded: items marked Pending have not been assessed, and open High findings prohibit internet/live exposure rather than being accepted as safe.
