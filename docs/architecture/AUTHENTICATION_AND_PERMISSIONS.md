...
`POST /auth/login` normalises email, verifies a bcrypt password hash and checks
active membership. STAFF/SCANNER receive the existing persisted eight-hour
...
Persisted privileged sessions additionally record MFA time, method and factor
generation. The JWT strategy requires that evidence to match the membership's
current active factor on every OWNER/MANAGER request.
...
Current-session and all-session revocation are server-authoritative. Sprint 33
adds local privileged MFA; production readiness still requires approved email
password recovery, managed MFA secret custody, monitoring, verified edge limits
and independent security review. Controlled ownership transfer and OWNER
lost-factor recovery remain separate from ordinary Team management.
